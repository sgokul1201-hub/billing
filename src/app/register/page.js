...
export default function RegisterPage() {
  const [activeTab, setActiveTab] = useState('profile'); // profile, inventory, customers, backup
  ...
  const loadCustomers = async () => {
    ...
    // Group by customer phone, aggregate stats
    const custMap = {};
    sales.forEach(sale => {
    ...
    // Convert map to array and sort by spend desc
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (confirm("WARNING: Importing this backup will OVERWRITE all current local database entries. Do you wish to continue?")) {
          ...
          // Restore tables
          const { shop, sales, items } = backupData.data;
        ...
        {activeTab === 'backup' && (
          <div className="page-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                Download all your shop settings, items catalog, and billing invoice history records into a single JSON file. You can save this backup to your phone storage or transfer it to another device.
              </p>
            ...
            <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                ...
                <span style={{ color: 'var(--danger)', fontWeight: 'bold' }}> Warning: This will overwrite all current settings, catalog, and invoices on this device.</span>
              </p>
